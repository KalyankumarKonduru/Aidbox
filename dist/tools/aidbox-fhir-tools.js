...
exports.AidboxFHIRTools = void 0;
class AidboxFHIRTools {
    client;
    constructor(client) {
        this.client = client;
    }
                ...
                content: [{
                        ...
                        text: JSON.stringify({
                            ...
                            patient: {
                                ...
                                generalPractitioner: patient.generalPractitioner?.map((gp) => gp.display || gp.reference),
                                communication: patient.communication?.map((comm) => comm.language.text || comm.language.coding?.[0]?.display)
                            }
                        }, null, 2)
                    }]
            };
    ...
    async handleAidboxUpdatePatient(args) {
        try {
            ...
            if (args.given || args.family) {
                ...
                if (args.given)
                    existing.name[0].given = [args.given];
                if (args.family)
                    existing.name[0].family = args.family;
            }
            if (args.birthDate !== undefined)
                existing.birthDate = args.birthDate;
            ...
            if (args.phone || args.email) {
                ...
                if (args.phone) {
                    const phoneIndex = existing.telecom.findIndex((t) => t.system === 'phone');
                    if (phoneIndex >= 0) {
                ...
                if (args.email) {
                    const emailIndex = existing.telecom.findIndex((t) => t.system === 'email');
                    if (emailIndex >= 0) {
        ...
        try {
            ...
            const observations = (bundle.entry || []).map((entry) => {
                ...
                return {
                    ...
                    issued: obs.issued,
                    performer: obs.performer?.map((p) => p.display || p.reference)
                };
            });
            return {
                content: [{
    ...
    async handleAidboxGetConditions(args) {
        try {
            ...
            if (args.clinicalStatus)
                params['clinical-status'] = args.clinicalStatus;
            const bundle = await this.client.search('Condition', params);
            const conditions = (bundle.entry || []).map((entry) => {
    ...
    async handleAidboxGetEncounters(args) {
        try {
            ...
            const encounters = (bundle.entry || []).map((entry) => {
        ...
        try {
            ...
            if (args.startDateTime || args.endDateTime) {
                ...
                if (args.startDateTime)
                    encounter.period.start = args.startDateTime;
                if (args.endDateTime)
                    encounter.period.end = args.endDateTime;
            }
            if (args.reasonCode) {
    ...
    formatObservationValue(obs) {
        ...
        if (obs.components) {
            return obs.components.map((comp) => ({
                code: comp.code?.text,
...
exports.AidboxFHIRTools = AidboxFHIRTools;
//# sourceMappingURL=aidbox-fhir-tools.js.map